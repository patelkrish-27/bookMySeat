  ...
  return (
    <div className="page-fade min-h-screen pt-20" style={{ background: 'transparent' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {bookingError && (
          <div className="mb-6 px-4 sm:px-5 py-4 rounded-xl flex items-start gap-3 glass-panel"
            style={{ background: 'rgba(230,57,70,0.1)', borderColor: 'rgba(230,57,70,0.25)', color: '#ff8f97' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-8">
            <h1 className="font-display font-bold text-2xl sm:text-3xl mb-1" style={{ color: '#f0f0f8' }}>Select Your Seats</h1>
            <p className="text-sm mb-6 sm:mb-8" style={{ color: '#555570' }}>{movie.title}</p>

            {/* Screen indicator */}
            <div className="flex flex-col items-center mb-8 sm:mb-10">
              <div className="w-3/4 h-1.5 sm:h-2 rounded-full mb-2" style={{ background: 'linear-gradient(90deg, transparent, rgba(212,166,58,0.5), transparent)', boxShadow: '0 0 20px rgba(212,166,58,0.15)' }} />
              <span className="text-xs font-mono-dm tracking-widest uppercase" style={{ color: '#555570' }}>Screen</span>
            </div>

            {loading ? (
              <div className="text-center py-20 flex flex-col items-center gap-3" style={{ color: '#9999bb' }}>
              ...
              <div className="space-y-2 sm:space-y-3 overflow-x-auto pb-2">
                {rows.map(row => {
                  const rowSeats = seats.filter(s => s.row === row).sort((a, b) => a.num - b.num)
                  if (rowSeats.length === 0) return null
                  const type = rowSeats[0]?.type
                  return (
                    <div key={row} className="flex items-center gap-2 sm:gap-3 min-w-fit">
                      <span className="w-5 text-center text-xs font-mono-dm font-bold flex-shrink-0" style={{ color: '#555570' }}>{row}</span>
                        ...
                        {rowSeats.map((seat) => {
                          ...
                          return (
                            <button
                              ...
                              className="seat-btn w-7 sm:w-8 h-6 sm:h-7 rounded-t-xl text-xs font-mono-dm font-bold transition-all"
                              style={{
                                background: color,
                                border,
                                color: seat.status === 'selected' ? '#07070f' : seat.status === 'taken' ? '#333350' : '#9999bb',
                                cursor: (seat.status === 'taken' || booking) ? 'not-allowed' : 'pointer',
                                boxShadow: seat.status === 'selected' ? '0 0 12px rgba(212,166,58,0.3)' : 'none',
                              }}
                      ...
                      <span className="w-14 sm:w-16 text-right text-xs font-mono-dm flex-shrink-0" style={{ color: type === 'RECLINER' ? 'rgba(100,180,255,0.7)' : type === 'PREMIUM' ? 'rgba(150,100,255,0.7)' : '#555570' }}>
                        {type}
                      </span>
                    </div>
                  )
            ...
            {/* Legend */}
            <div className="flex items-center justify-center gap-4 sm:gap-8 mt-8 sm:mt-10 pt-6 flex-wrap" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
              {[
                ...
                <div key={l.label} className="flex items-center gap-2">
                  <div className="w-5 sm:w-6 h-4 sm:h-5 rounded-t-lg" style={{ background: l.color, border: `1px solid ${l.border}` }} />
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-24 glass-panel rounded-2xl p-5">
              ...
              <div className="flex gap-3 mb-5 p-3 rounded-xl glass-card">
              ...
              {selected.length > 0 ? (
                <div className="space-y-2 mb-5">
                  {selected.map(id => {
                    ...
                    return (
                      <div key={id} className="flex items-center justify-between py-2 px-3 rounded-lg" style={{ background: 'rgba(212,166,58,0.06)', border: '1px solid rgba(212,166,58,0.12)' }}>
                        <div>
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 active:scale-95 ${selected.length > 0 && !booking ? 'glass-btn-gold' : ''}`}
                style={{
                  background: (selected.length > 0 && !booking) ? undefined : 'rgba(255,255,255,0.06)',
                  color: (selected.length > 0 && !booking) ? undefined : '#555570',
                  cursor: (selected.length === 0 || booking) ? 'default' : 'pointer',
                }}
              >
                {booking
                  ? 'Reserving seats…'
